 ...
 */
DROP TABLE BequestData;
CREATE TABLE BequestData NOLOGGING AS
...
BequestData_CTE as (SELECT
    /*add all available fields*/
    ai.roi_id,
    ai.roi_family_id,
    NTILE(4) OVER (ORDER BY ROI_Family_ID) as quartile,
    ai.first_name,
    ai.middle_name,
    ai.last_name,
    ai.name_suffix,
...
SELECT *
FROM BequestData_CTE
WHERE quartile = 1;

COMMIT;

CALL ROI_OUTPUT_TABLE('OWNER','BequestData1','csv','jhaybok@npca.org,dbouquin@npca.org');
